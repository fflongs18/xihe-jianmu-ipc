...
db.exec(`
  ...
  CREATE INDEX IF NOT EXISTS idx_pairing_expiry ON pairing_sessions(expires_at);
...
ensureColumn('pairing_sessions', 'failed_attempts', 'failed_attempts INTEGER NOT NULL DEFAULT 0');
ensureColumn('pairing_sessions', 'locked_at', 'locked_at INTEGER');
...
const insertPairingSession = db.prepare('INSERT INTO pairing_sessions (id,code_hash,node_name,expires_at,created_at) VALUES (@id,@codeHash,@nodeName,@expiresAt,@createdAt)');
const insertNodeAudit = db.prepare('INSERT INTO node_audit (node_id,event,details,ts) VALUES (?,?,?,?)');

export function createPairingSession({ nodeName, ttlMs = 10 * 60 * 1000 } = {}) {
  const id = `pair_${randomBytes(8).toString('hex')}`;
  const code = randomCode();
  const now = Date.now();
  const normalizedName = String(nodeName || 'Remote Client').trim().slice(0, 128);
  insertPairingSession.run({ id, codeHash: hashSecret(code), nodeName: normalizedName, expiresAt: now + ttlMs, createdAt: now });
  insertNodeAudit.run(null, 'pair.create', JSON.stringify({ pairingId: id, nodeName: normalizedName, expiresAt: now + ttlMs }), now);
  return { id, code, nodeName: normalizedName, expiresAt: now + ttlMs };
}

export function exchangePairingSession({ id, code, platform, clientVersion } = {}) {
  const normalizedCode = String(code || '').trim().toUpperCase();
  const pairingId = typeof id === 'string' && id.startsWith('pair_') ? id : null;
  const now = Date.now();
  return db.transaction(() => {
    const row = pairingId
      ? db.prepare('SELECT * FROM pairing_sessions WHERE id = ?').get(pairingId)
      : db.prepare('SELECT * FROM pairing_sessions WHERE code_hash = ?').get(hashSecret(normalizedCode));
    if (!row) return { ok: false, reason: 'invalid' };
    if (row.locked_at) return { ok: false, reason: 'locked' };
    if (row.consumed_at) return { ok: false, reason: 'consumed' };
    if (row.expires_at < now) return { ok: false, reason: 'expired' };
    if (hashSecret(normalizedCode) !== row.code_hash) {
      const failures = Number(row.failed_attempts || 0) + 1;
      const lockedAt = failures >= 5 ? now : null;
      db.prepare('UPDATE pairing_sessions SET failed_attempts = ?, locked_at = COALESCE(locked_at, ?) WHERE id = ?')
        .run(failures, lockedAt, row.id);
      insertNodeAudit.run(null, 'pair.failure', JSON.stringify({ pairingId: row.id, reason: lockedAt ? 'locked' : 'invalid_code', failures }), now);
      return { ok: false, reason: lockedAt ? 'locked' : 'invalid' };
    }

    const consumed = db.prepare(`UPDATE pairing_sessions SET consumed_at = ?
      WHERE id = ? AND consumed_at IS NULL AND locked_at IS NULL AND expires_at >= ?`).run(now, row.id, now);
    if (consumed.changes !== 1) return { ok: false, reason: 'unavailable' };

    const nodeId = `remote-oc-${randomBytes(6).toString('hex')}`;
    const token = `jnt_${randomBytes(32).toString('base64url')}`;
    db.prepare(`INSERT INTO node_credentials (node_id,node_name,token_hash,session_name,platform,client_version,created_at,last_seen,token_status,runtimes)
      VALUES (@nodeId,@nodeName,@tokenHash,@nodeId,@platform,@clientVersion,@createdAt,@lastSeen,'active','[]')`)
      .run({ nodeId, nodeName: row.node_name, tokenHash: hashSecret(token), platform: platform || null, clientVersion: clientVersion || null, createdAt: now, lastSeen: now });
    insertNodeAudit.run(nodeId, 'pair.exchange', JSON.stringify({ pairingId: row.id }), now);
    return { ok: true, nodeId, nodeToken: token, nodeName: row.node_name, createdAt: now };
  })();
}

export function authenticateNodeToken(token, sessionName = null) {
  if (!token) return null;
  const row = db.prepare('SELECT * FROM node_credentials WHERE token_hash = ? AND token_status = \'active\'').get(hashSecret(token));
  if (!row || (sessionName && row.session_name && row.session_name !== sessionName)) return null;
  return row;
}

export function recordNodeAudit(nodeId, event, details = null) {
  insertNodeAudit.run(nodeId || null, event, details ? JSON.stringify(details) : null, Date.now());
}

export function listNodes() {
  return db.prepare('SELECT * FROM node_credentials ORDER BY created_at DESC').all().map((row) => ({ ...row, online: Boolean(row.online), runtimes: JSON.parse(row.runtimes || '[]') }));
}
export function getNode(nodeId) {
  const row = db.prepare('SELECT * FROM node_credentials WHERE node_id = ?').get(nodeId);
  return row ? { ...row, online: Boolean(row.online), runtimes: JSON.parse(row.runtimes || '[]') } : null;
}
export function updateNodeManifest(nodeId, manifest = {}) {
  const now = Date.now();
  db.prepare(`UPDATE node_credentials SET node_name=COALESCE(@nodeName,node_name),session_name=COALESCE(@sessionName,session_name),platform=@platform,client_version=@clientVersion,last_seen=@lastSeen,connected_at=COALESCE(connected_at,@lastSeen),online=1,runtimes=@runtimes WHERE node_id=@nodeId`).run({ nodeId, nodeName: manifest.label || null, sessionName: manifest.sessionName || null, platform: manifest.platform || null, clientVersion: manifest.clientVersion || null, lastSeen: now, runtimes: JSON.stringify(manifest.runtimes || []) });
  return getNode(nodeId);
}
export function markNodeOffline(sessionName) { db.prepare('UPDATE node_credentials SET online=0,last_seen=? WHERE session_name=?').run(Date.now(), sessionName); }
export function revokeNode(nodeId) { const result = db.prepare("UPDATE node_credentials SET token_status='revoked',online=0 WHERE node_id=?").run(nodeId); if (result.changes > 0) recordNodeAudit(nodeId, 'node.revoke'); return result.changes > 0; }
export function renameNode(nodeId, nodeName) { const result = db.prepare('UPDATE node_credentials SET node_name=? WHERE node_id=?').run(String(nodeName).trim().slice(0,128), nodeId); return result.changes > 0 ? getNode(nodeId) : null; }
